...
## Purpose
This document provides a comprehensive analysis of the **current showcase/dummy frontend** implementation for the Storytelling Pipeline. It explains how the current frontend displays pipeline outputs, the API calls made, data flow, and integration details.

**Note**: This document describes the existing (placeholder) frontend. Real frontend developers building the production frontend should refer to `frontend_developer_guide.md` instead.
...
**This is a DUMMY/SHOWCASE frontend** - not the production frontend. It was built to demonstrate the backend capabilities but is not the real product UI.

### WAT Framework (Backend Only)
The **WAT (Workflows, Agents, Tools)** architecture applies to the **backend AI pipeline**, not this frontend. WAT is defined in `Complete_Architecture.md`:

- **Workflows**: Pipeline orchestration (LangChain)
- **Agents**: AI reasoning (GPT-4o-mini)
- **Tools**: Code execution (Python)

The current frontend simply displays outputs generated by the WAT-based backend pipeline.
